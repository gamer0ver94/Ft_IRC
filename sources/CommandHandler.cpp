...
bool CommandHandler::parseNickNameMessage(const std::string& message, std::string& nickName, std::string& username, std::string& hostName, std::string& serverHostName, std::string& realName, std::string& password) {
    ...
    for (size_t i = 0; i < words.size(); i++) {
        if (i + 1 < words.size()) {  // Check if there is enough space for the next word
            if (words[i] == "PASS") {
                password = words[i + 1];
            } else if (words[i] == "NICK") {
                nickName = words[i + 1];
            } else if (words[i] == "USER") {
                if (i + 4 < words.size()) {  // Check if there are enough words for USER command
                    username = words[i + 1];
                    hostName = words[i + 2];
                    ...
                    return false;  // Not enough words for USER command
                }
            }
        }
    }
...
int CommandHandler::getMessageType(std::string message){
    if (message.find("CAP LS") != std::string::npos || message.find("PASS") != std::string::npos || message.find("NICK") != std::string::npos || message.find("USER") != std::string::npos){
        return 0;
    }
...
std::string CommandHandler::handleAuthentication(std::string message, Server &server, int clientFd){
    std::istringstream iss(message);
    std::string command;
